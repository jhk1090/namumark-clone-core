...
function sha224Replace(data) {
    return node_crypto_1.default.createHash("sha224").update(data, "utf-8").digest("hex");
}
...
class NamuMark {
    ...
    database;
    tempALinkCount;
    constructor(wikiText, config, database) {
    ...
    getToolDataRestore = (data, doType = "all") => {
        let storageCount = this.dataTempStorageCount * 3;
        const storageRegex = doType === "all" ? /<(\/?(?:render|slash)_(?:[0-9]+))>/ : doType === "render" ? /<(\/?(?:render)_(?:[0-9]+))>/ : /<(\/?(?:slash)_(?:[0-9]+))>/;
        while (true) {
            if (!data.match(storageRegex))
                break;
            if (storageCount < 0) {
                ...
                data = data.replace(storageRegex, (match, p1) => this.dataTempStorage[p1]);
            }
            storageCount -= 1;
        }
        return data;
    };
    getToolDataRevert = (data, doType = "all") => {
        let storageCount = this.dataTempStorageCount * 3;
        const storageRegex = doType === "all"
            ...
            if (includeCountMax < 0) {
                ...
                if (this.docInclude != "") {
                    ...
                    if (dbData) {
                        ...
                        dataName = this.getToolDataStorage(`${includeLink}<div id="${this.docInclude}opennamu_include_${includeNum}"></div>`, "", matchOriginal);
                    }
                    else {
            ...
            if (middleCountAll < 0) {
                ...
                if (middleName) {
                    ...
                    if (middleNameNew === "#!wiki") {
                        ...
                        dataName = this.getToolDataStorage("<details><summary>", '</summary><div id="' + this.docInclude + "opennamu_folding_" + foldingCount + '"></div></details>', middleDataOriginal);
                        foldingCount += 1;
                    }
                        ...
                        dataName = this.getToolDataStorage('<pre id="syntax"><code class="' + wikiDataSyntax + '">' + wikiData, "</code></pre>", middleDataOriginal);
                        syntaxCount += 1;
                    }
                        ...
                        if (middleNameNew === "+5")
                            wikiSize = "200";
                        else if (middleNameNew === "+4")
                            wikiSize = "180";
                        else if (middleNameNew === "+3")
                            wikiSize = "160";
                        else if (middleNameNew === "+2")
                            wikiSize = "140";
                        else
                            wikiSize = "120";
...
try {
  ...
  if (document.getElementById("${nameOb}")) {
    ...
    manageTable() {
        this.wikiText = this.wikiText.replace(/\n +\|\|/g, "\n||");
        const manageTableParameter = (cellCount, parameter, data, option = {}) => {
            ...
            for (const tableParameter of parameter.match(tableParameterRegex) || []) {
                ...
                if (tableParameterSplit.length === 2) {
                    ...
                    if (tableParameterName === "tablebgcolor")
                        ...
                        tableParameterAll["table"] += "width:" + this.getToolPxAddCheck(tableParameterData) + ";";
                        ...
                        tableParameterAll["table"] += "text-align:" + tableParameterData + ";";
                        ...
                        tableParameterAll["tr"] += "text-align:" + tableParameterData + ";";
        ...
        while (true) {
            ...
            if (tableCountAll < 0) {
                ...
                for (const tableSub of results) {
                    ...
                    if (tableDataEnd === "")
                        tableDataEnd += '<tr style="' + tableSubParameter["tr"] + '">';
    ...
    manageMacro() {
        ...
        const manageMacroDouble = (match, p1, p2) => {
            ...
            if (["youtube", "nicovideo", "navertv", "kakaotv", "vimeo"].includes(nameData)) {
                ...
                for (const datum of data) {
                    ...
                    if (dataSub) {
                        ...
                        if (dataSub[1] === "width")
                            videoWidth = this.getToolPxAddCheck(dataSub[2]);
                        else if (dataSub[1] === "height")
                            videoHeight = this.getToolPxAddCheck(dataSub[2]);
                        else if (dataSub[1] === "start")
                            videoStart = dataSub[2];
                        else if (dataSub[1] === "end")
                            videoEnd = dataSub[2];
                        else if (dataSub[1] === "https://www.youtube.com/watch?v" && nameData == "youtube")
                            videoCode = dataSub[2];
                    }
                    else {
                        videoCode = datum;
                ...
                const dataName = this.getToolDataStorage('<iframe style="width: ' + videoWidth + "; height: " + videoHeight + ';" src="' + videoCode + '" frameborder="0" allowfullscreen>', "</iframe>", matchOriginal);
                return "<" + dataName + "></" + dataName + ">";
            }
        ...
        const manageMacroSingle = (match, p1) => {
            const matchOriginal = match;
            p1 = p1.toLowerCase();
    ...
    manageLink() {
        ...
        while (true) {
            if (!this.wikiText.match(linkRegex))
                break;
                ...
                if (linkMain.match(/^(파일|file|외부|out):/i)) {
                    ...
                    // 링크 존재 여부에 따른 처리
                    if (linkExist !== "") {
                        const dataName = this.getToolDataStorage(`<a class="${linkExist}" title="${linkSub}" href="/upload?name=${urlPas(linkMainOriginal)}">${linkSub}</a>`, "</a>", linkDataFull);
                        ...
                        const dataName = filePass === 1
                            ? fileOut === 0
                                ? this.getToolDataStorage('<a title="' + linkSub + '" href="/w/file:' + urlPas(linkMainOriginal) + "." + urlPas(linkExtensionNew) + '">' + fileEnd, "</a>", linkDataFull)
                ...
                // out link 처리
                    ...
                    const dataName = this.getToolDataStorage(`<a class="opennamu_link_out" target="_blank" title="${linkTitle}" href="${linkMain}">${linkSubStorage}</a>`, "", linkDataFull);
                ...
                // in link 처리
                    ...
                    let linkSub = linkData[1] || "";
                    const linkSubStorage = linkSub ? "" : linkMainOriginal;
                    const dataName = this.getToolDataStorage(`<a class="${linkExist} ${linkSame}" title="${linkTitle}" href="${linkMain}${linkDataSharp}">${linkSubStorage}</a>`, "", linkDataFull);
                    this.wikiText = this.wikiText.replace(linkRegex, `<${dataName}>${linkSub}</${dataName}>`);
                }
            }
            linkCountAll -= 1;
            ...
            if (footnoteCountAll < 0) {
                ...
                if (footnoteDataGroups[2]) {
                    ...
                    if (this.dataFootnote[footnoteName]) {
                        ...
                        const dataName = this.getToolDataStorage(`<sup><a fn_target="${this.docInclude}fn_${footnoteFirst}" id="${this.docInclude}rfn_${footnoteNumStr}" href="#${this.docInclude}fn_${footnoteFirst}">(${footnoteName} (${footnoteNumStr}))</a></sup>`, "", footnoteDataOriginal);
                        ...
                        const dataName = this.getToolDataStorage(`<sup><a fn_target="${this.docInclude}fn_${footnoteNumStr}" id="${this.docInclude}rfn_${footnoteNumStr}" href="#${this.docInclude}fn_${footnoteNumStr}">(${footnoteName}${footnoteNameAdd})</a></sup>`, "", footnoteDataOriginal);
                        this.wikiText = this.wikiText.replace(footnoteRegex, `<${dataName}></${dataName}>`);
                    }
                }
                footnoteCountAll -= 1;
    ...
    manageText() {
        // <b> function
        const doRenderTextBold = (match, ...groups) => {
            const data = groups[0];
            let dataName;
        ...
        // <i> function
        const doRenderTextItalic = (match, p1) => {
            const data = p1;
        ...
        // <u> function
        const doRenderTextUnder = (match, p1) => {
            const data = p1;
        ...
        // <sup> function
        const doRenderTextSup = (match, p1) => {
            const data = p1;
        ...
        this.wikiText = this.wikiText.replace(/\^\^((?:(?!\^\^).)+)\^\^/g, doRenderTextSup);
        // <sub> function
        const doRenderTextSub = (match, p1) => {
            const data = p1;
        ...
        this.wikiText = this.wikiText.replace(/,(?:(?!,,).)+,/g, doRenderTextSub);
        // <s> function
        const doRenderTextStrike = (match, p1) => {
            const data = p1;
            let dataName;
        ...
        while (true) {
            ...
            if (!headingRegex.exec(this.wikiText)) {
                ...
                `;
                    ...
                    const dataName = this.getToolDataStorage(`<h${headingLevelStr}>`, `<sub><a id="${this.docInclude}editLoad_${headingCount}" href="/edit_section/${headingCount}/${urlPas(this.docName)}">✎</a><a href="javascript:void(0);" onclick="javascript:opennamuHeadingFolding('${this.docInclude}opennamuHeading_${headingCount}', this);">${headingFolding[0]}</a></sub></h${headingLevelStr}>`, headingDataOriginal);
    ...
    finalize() {
        ...
        const handlerFootnote = (match, p1) => {
